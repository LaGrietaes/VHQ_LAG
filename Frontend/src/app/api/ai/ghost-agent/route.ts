...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('=== GHOST AGENT: Processing request ===');
    ...
    const messages = [
        { 
            role: 'system', 
            content: `Eres un asistente experto que responde siempre en español.
...
REGLAS CRÍTICAS:
- SOLO puedes usar la información del contexto de arriba
- NUNCA uses conocimiento previo o información externa
- Si te preguntan sobre algo del contexto, busca esa información específica en el texto
- Si no encuentras la información, di "No encuentro esa información en el contexto"
- El contexto es tu ÚNICA fuente de información válida

IMPORTANTE: Cuando te pregunten sobre el contenido del contexto, busca la información específica en el texto y responde SOLO con esa información. No inventes nada.`
        });
    }
    ...
    console.log('Context content length:', contextContent ? contextContent.length : 0);
    console.log('Context preview:', contextContent ? contextContent.substring(0, 200) + '...' : 'No context');
    ...
    console.error('Error en la ruta ghost-agent:', error);
