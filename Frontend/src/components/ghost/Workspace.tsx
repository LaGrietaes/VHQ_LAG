...
type ProjectWorkspaceProps = {
  project: Project;
  projectType: 'scripts' | 'blogs' | 'books' | 'socials';
  onBack: () => void;
  onUpdateProject: (project: Project) => void;
  allProjects: Project[];
};

export const ProjectWorkspace = ({ project, projectType, onBack, onUpdateProject, allProjects }: ProjectWorkspaceProps) => {
  ...
  }, [project, editingMode, projectType]);

  useEffect(() => {
    if (editingMode === 'project') {
      if(projectType !== 'socials') {
  ...
  }, [content, platformContents, project, editingMode, projectType]);
  ...
  const handleSave = () => {
    console.log("Saving project content:", content, platformContents);
    setIsDirty(false);
  ...
  const saveNewTemplate = () => {
    // For now, it requires a title. We can prompt for it later.
    const title = newTemplateContent.split('\n')[0].replace('#', '').trim() || "Nueva Plantilla";
    const newTemplate = { title, content: newTemplateContent };

    setAllTemplates(prev => {
        const currentCategoryTemplates = prev[projectType];
        return {
            ...prev,
            [projectType]: [...currentCategoryTemplates, newTemplate]
        }
    });
  ...
  const handleTemplateSelect = (templateContent: string, platform?: Platform) => {
    ...
    if (projectType === 'socials') {
        if (platform) {
            // Apply to specific platform if it's selected
  ...
  const handleEditorChange = (value: string, platform?: Platform) => {
      ...
      } else if (projectType === 'socials' && platform) {
  ...
  const handleSocialPlatformToggle = (platform: Platform) => {
    ...
    if (!newPlatforms.includes(platform)) {
        // remove content when unselecting
  ...
  const getLowestCharLimit = () => {
    if (!project.socialPlatforms || project.socialPlatforms.length === 0) return null;

    const limits = project.socialPlatforms
      .map(p => platformCharLimits[p])
      .filter((limit): limit is number => limit !== undefined);

    return limits.length > 0 ? Math.min(...limits) : null;
  }

  const lowestLimit = getLowestCharLimit();
    ...
    <div className="h-full flex flex-col bg-black text-gray-300 font-mono">
      ...
      <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-gray-800">
        <div className="flex items-center gap-4">
          ...
          {editingMode === 'project' && projectType !== 'socials' && (
            <>
      ...
      <div className="flex-grow flex h-[calc(100%-73px)]">
        {/* Left Panel: Outline - Conditionally rendered */}
        {projectType !== 'socials' && (
                     ...
                     <DropdownMenu>
                        ...
                        <DropdownMenuContent className="w-[var(--radix-dropdown-menu-trigger-width)] bg-gray-900 border-gray-700 text-gray-300" side="top">
                            <DropdownMenuLabel className="flex items-center gap-2 text-red-500">
                              <FilePlus className="h-4 w-4" /> PLANTILLAS
                            </DropdownMenuLabel>
        ...
        {/* Right Panel: Editor and Integration */}
        <main className={cn(
            "flex flex-col bg-black",
            projectType === 'socials' ? "w-full" : "w-2/3 lg:w-3/4"
        )}>
           {projectType === 'socials' && editingMode === 'project' ? (
            <div className="p-4 flex-grow">
                 {(!project.socialPlatforms || project.socialPlatforms.length === 0) ? (
                    ...
                    <PanelGroup direction="horizontal" className="h-full w-full">
                                        ...
                                        <Panel key={platform} minSize={20}>
                                             <div className="bg-gray-900/50 rounded-lg border border-gray-800 h-full overflow-hidden flex flex-row">
                                                ...
                                                {activeEmojiPicker === platform && (
                                                    <div className="flex-shrink-0 border-l border-gray-800 bg-black/20" style={{'--epr-bg-color': 'transparent'}}>
                                                        <EmojiPicker 
                        ...
                        <PanelResizeHandle className="w-1 h-full bg-gray-800 hover:bg-red-600 transition-colors" />
                        <Panel defaultSize={50} minSize={25}>
                             <PanelGroup direction="vertical">
            ...
            <footer className="flex-shrink-0 p-4 border-t border-gray-800 flex justify-between items-stretch gap-4">
               <div className="flex items-center gap-2">
                 {editingMode === 'project' && projectType === 'socials' && (
                  <>
                     <div className="flex items-center gap-1 p-1 border border-gray-700 rounded-md h-full">
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="outline" className="h-full border-gray-700 hover:border-red-600 hover:bg-gray-900 hover:text-red-400 flex items-center justify-center gap-2 px-4">
                                <FilePlus className="h-4 w-4" />
                                <span>PLANTILLAS</span>
                                <ChevronsUp className="h-4 w-4" />
                            </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent className="w-64 bg-gray-900 border-gray-700 text-gray-300" side="top">
                            <DropdownMenuLabel className="flex items-center gap-2 text-red-500">
                              <FilePlus className="h-4 w-4" /> PLANTILLAS
                            </DropdownMenuLabel>
                            <DropdownMenuSeparator className="bg-gray-700"/>
                                ...
                                return (
                                    <DropdownMenuSub key={platform}>
                                        <DropdownMenuSubTrigger className="hover:bg-gray-800 hover:text-white focus:bg-gray-700">
                                            <span>Plantillas para {platform.charAt(0).toUpperCase() + platform.slice(1)}</span>
                                        </DropdownMenuSubTrigger>
                                        <DropdownMenuPortal>
                            ...
                            {/* General templates */}
                            <DropdownMenuSeparator className="bg-gray-700"/>
                            <DropdownMenuLabel className="text-gray-400 font-normal">Generales (Aplicar a Todos)</DropdownMenuLabel>
                             {socialTemplates.filter(t => !t.platform).map(template => (
                                 <DropdownMenuItem 
                                    key={template.title}
                                    onSelect={() => handleTemplateSelect(template.content)}
                                    className="hover:bg-gray-800 hover:text-white"
